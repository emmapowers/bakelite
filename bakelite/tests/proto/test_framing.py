...
def describe_encoder():
    ...
    def encode_null_terminated(expect):
        expect(framing.encode(b"hello\x00world\x00")) == b"\x06hello\x06world\x01"
    ...
    def encode_decode(expect):
        expect(framing.decode(framing.encode(b"Hello\x00world!"))) == b"Hello\x00world!"
...
def describe_crc():
    ...
    def append_crc_8bit(expect):
        expect(framing.append_crc(b"hello world", crc_size=CrcSize.CRC8)) == b"hello world\xa8"

    def check_crc_8bit(expect):
        expect(framing.check_crc(b"hello world\xa8", crc_size=CrcSize.CRC8)) == b"hello world"

    def append_crc_16bit(expect):
        expect(framing.append_crc(b"hello world", crc_size=CrcSize.CRC16)) == b"hello world\xc19"

    def check_crc_16bit(expect):
        expect(framing.check_crc(b"hello world\xc19", crc_size=CrcSize.CRC16)) == b"hello world"

    def append_crc_32bit(expect):
        expect(
            framing.append_crc(b"hello world", crc_size=CrcSize.CRC32)
...
def describe_framer():
    def encode_frame(expect):
        ...
        expect(framer.encode_frame(b"hello\x00world")) == b"\x00\x06hello\x07world\x93\x00"

    def encode_frame_no_crc(expect):
        framer = framing.Framer(crc=CrcSize.NO_CRC)
        expect(framer.encode_frame(b"hello\x00world")) == b"\x00\x06hello\x06world\x00"

    def encode_frame_crc_32(expect):
        framer = framing.Framer(crc=CrcSize.CRC32)
        expect(framer.encode_frame(b"hello\x00world")) == b"\x00\x06hello\nworld\xb3\x14\xe6\n\x00"
