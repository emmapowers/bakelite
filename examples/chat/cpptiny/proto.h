...
public:
  ...
  int send(const ChatMessage*) {
    m_framer.buffer()[0] = static_cast<char>(Message::ChatMessage);
    size_t frameSize = sizeof(ChatMessage) + 1;
  ...
  int send(const SetName*) {
    m_framer.buffer()[0] = static_cast<char>(Message::SetName);
    size_t frameSize = sizeof(SetName) + 1;
    auto result = m_framer.encodeFrame(frameSize);
  ...
  int send(const ChatMessage &val) {
