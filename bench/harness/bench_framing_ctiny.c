/*
 * Benchmark harness for framing.bakelite (ctiny)
 *
...
void bench_Payload_pack(void) {
    bakelite_buffer_reset(&g_buf);
    g_result = Payload_pack(&g_payload_in, &g_buf);
}

void bench_Payload_unpack(void) {
    bakelite_buffer_reset(&g_buf);
    g_result = Payload_unpack(&g_payload_out, &g_buf);
}
